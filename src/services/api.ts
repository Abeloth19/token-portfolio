...
export const fetchTokenDetails = async (tokenId: string): Promise<TokenPrice> => {
  try {
...
export const fetchPriceHistory = async (tokenId: string, days = 7): Promise<number[]> => {
  try {
    const id = getCoinGeckoId(tokenId)
    const response = await fetch(
...
export const generateSparklinePath = (prices: number[], width = 76, height = 28): string => {
